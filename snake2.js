...
let gameOver = false;
let canMove = true;
let swDrawDist = false;
let swDrawGrid = false;
...
// Desenha a cobrinha
function drawSnake() {
	let green = maxColorSnake;
	let blue = minColorSnake;
	let part = (maxColorSnake - minColorSnake) / (snake.length / divisor);
	for (let i = 0; i < snake.length; i++) {
		ctx.fillStyle = "rgb(0, " + green + ", " + blue + ")";
		ctx.fillRect(snake[i].x * size, snake[i].y * size, size, size);
		green -= part;
		blue += part;
		if (green < minColorSnake) green = minColorSnake;
		if (blue > maxColorSnake) blue = maxColorSnake;
	}
}
...
document.onkeydown = function (event) {
	switch (event.keyCode) {
		...
		case 68:
			swDrawDist = swDrawDist ? false : true;
			break;

		case 71:
			swDrawGrid = swDrawGrid ? false : true;
			break;
	}
};
